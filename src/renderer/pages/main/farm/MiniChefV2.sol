...
contract MiniChefV2 is BoringOwnable, BoringBatchable {
    ...
    address constant WSAFE = 0x0000000000000000000000000000000000001101;
    ...
    receive() external payable {
        assert(msg.sender == WSAFE); // only accept ETH via fallback from the WETH contract
    }
    ...
    function harvest(uint256 pid, address to) public {
        ...
        if (_pendingSushi != 0) {
           ...
           if (address(SUSHI) == WSAFE) {

              // 解包为原生 SAFE

              IWSAFE(WSAFE).withdraw(_pendingSushi);

              TransferHelper.safeTransferETH(to, _pendingSushi);
