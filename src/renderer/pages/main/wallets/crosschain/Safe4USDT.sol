...
contract Safe4USDT is TetherToken {
    ...
    // 构造函数
    function Safe4USDT(uint _initialSupply, string _name, string _symbol, uint8 _decimals)
    ...
    // 跨链发行USDT（带交易ID防重复）
    function crossChainIssue(address _to, uint amount, string _network, string _txId) public onlyOwner whenNotPaused {
        ...
        // 触发事件
        Issue(amount);
        CrossChainIssue(_to, amount, _network, _txId);
        Transfer(address(0), _to, amount);
    }

    // 跨链赎回USDT（带目标地址）
    function crossChainRedeem(uint amount, string _network, string _to) public whenNotPaused {
        ...
        // 触发跨链事件
        CrossChainRedeem(msg.sender, _to, amount, _network);
        Transfer(msg.sender, address(0), amount);
    }

    // 添加新网络支持
    function addNetwork(string _network) public onlyOwner {
        ...
        NetworkAdded(_network);
    }

    // 移除网络支持
    function removeNetwork(string _network) public onlyOwner {
        require(supportedNetworks[_network], "Network not supported");
        supportedNetworks[_network] = false;
        NetworkRemoved(_network);
    }

    // 检查网络是否支持
    function isNetworkSupported(string _network) public view returns (bool) {
        return supportedNetworks[_network];
    }

    // 检查交易ID是否已处理
    function isTxIdProcessed(string _txId) public view returns (bool) {
        return bytes(processedTxIds[_txId]).length > 0;
    }

    // 获取交易ID对应的网络
    function getTxIdNetwork(string _txId) public view returns (string) {
        return processedTxIds[_txId];
    }

    // 紧急功能：重置交易ID状态（仅限合约所有者）
    function resetTxId(string _txId) public onlyOwner {
        processedTxIds[_txId] = "";
    }


    // 统计查询函数
    function getNetworkStats(string _network) public view returns (
        uint256 issueCount,
        uint256 redeemCount,
        uint256 issueAmount,
    ...
    function getAllNetworksStats() public view returns (
        string[4] networks,
        uint256[4] issueCounts,
        uint256[4] redeemCounts,
        uint256[4] issueAmounts,
        uint256[4] redeemAmounts
    ) {
        networks[0] = "eth";
        networks[1] = "bsc";
        networks[2] = "tron";
    ...
    // 记录查询函数
    function getIssueRecord(string _txId) public view returns (
        address to,
        uint256 amount,
        string network,
        string txId,
    ...
    // 检查Issue记录是否存在
    function isIssueRecordExists(string _txId) public view returns (bool) {
        IssueRecord memory record = issueRecords[_txId];
